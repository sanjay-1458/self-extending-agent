...
const capabilityKey = (name: string) =>
...
export async function loadCapability(
  ...
  return raw
    ? CapabilitySchema.parse(JSON.parse(raw))
    : null;
}
...
export async function listCapabilities():
  ...
  const names =
    await redis.sMembers(CAPABILITY_SET);

  const items =
    await Promise.all(
      names.map(loadCapability)
    );

  return items.filter(
    (x): x is Capability => Boolean(x)
  );
}
